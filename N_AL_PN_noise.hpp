...
class PN : public Neuron {
 public:
  void ode_set(state_type &variables, state_type &dxdt,const double t, unsigned index) {
    ...
    double P = engine::neuron_value(index, "I_PeriodicPulse");   	// The external current provided to all neurons

    double current1 = 0;
    current1 = noise::inject(index);
    double ampNoise = engine::neuron_value(index, "ampNoise");
    double I_Noise = ampNoise*current1*(v+0.0);
    ...
    dxdt[v_index] = I_ext + current - I_Na - I_K - I_A - I_l - I_kl - I_syn - I_Noise + P;
...
//    dxdt[v_index] = I_ext + current - I_Na - I_K - I_A - I_l - I_kl - I_syn;
